...
- TIMWE outbound contract sharp edge (`services/subscription-external/internal/service/subscription.go`):
  - Expected: `status` requests follow the same required headers as `optin/optout/confirm` from `docs/SM External Rest API.postman_collection.json`, including `external-tx-id`.
  - Happened: `sendStatusCheckWithRetry` sent `apikey/authentication/content-type` but omitted `external-tx-id` (and `Accept`), while other flows set it.
  - Diagnosis: Compared Postman collection request definitions for `optin`, `optout`, `status`, and `optin/confirm` against sender implementations.
  - Fix/workaround: Added unified Postman-shaped payload builders + common headers (`apikey`, `authentication`, `external-tx-id`, `Content-Type`, `Accept`) for all four flows.
  - Prevention: Keep and extend request contract tests in `services/subscription-external/internal/service/subscription_request_contract_test.go` and header retry tests in `services/subscription-external/internal/service/subscription_external_tx_id_test.go`.

- TIMWE confirm edge case (`services/acquisition-api/internal/service/transaction_service.go`):
  - Expected: confirm payload follows Postman contract and sends only `productId` (no `pricepointId` field).
  - Happened: fallback logic that swapped confirm `productId` to `pricepoint_id` after `INVALID_PRICEPOINT_ID` caused drift from the known working request shape.
  - Diagnosis: Compared successful Postman confirm payload against acquisition confirm fallback path and verified contract fields.
  - Fix/workaround: Removed fallback substitution; confirm now always uses transaction-scoped offer `productId`.
  - Prevention: Keep test `TestConfirmTransaction_DoesNotRetryWithPricepointOnInvalidPricepointID` in `services/acquisition-api/internal/service/transaction_service_test.go` and keep per-attempt header uniqueness tests in `services/acquisition-api/internal/service/timwe_client_test.go`.

- Subscription-external repository interface sharp edge (`services/subscription-external/internal/repository/subscription.interface.go`):
  - Expected: adding a few new methods to `SubscriptionRepositoryInterface` is low-risk.
  - Happened: the interface is implemented by many test mocks across `internal/service/*_test.go`, so adding methods creates large compile fallout.
  - Diagnosis: `rg` of `MockSubscriptionRepository` usages showed many mock structs depend on the existing interface shape.
  - Fix/workaround: keep core interface stable and use a narrow local interface + type assertion in handlers for new optional repository capabilities.
  - Prevention: for additive features, prefer local capability interfaces in handlers/services unless a cross-cutting contract change is explicitly required.
