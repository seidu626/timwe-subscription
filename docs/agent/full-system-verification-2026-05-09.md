...
|---|---|---|
| Verify local and remote `main` as one integrated state | Local and remote main histories conflict heavily. The 2026-05-09T08:44:16Z evidence snapshot showed primary `main` clean but diverged from `origin/main` (`main` `ab22b15`, `origin/main` `bad5fe1`). Exact behind counts change as `origin/main` receives evidence-only commits. | Human-directed integration strategy for `main...origin/main`; the clean PR branch intentionally uses `origin/main` as source of truth. |
...
- Local and remote `main` diverge with overlapping histories; the 2026-05-09T08:44:16Z evidence snapshot showed primary `main` clean but diverged from `origin/main`, and exact behind counts change as `origin/main` receives evidence-only commits.
