...
| docker compose dev stack | fixed acquisition image build and notification-worker startup | n/a | blocked by service runtime config/schema | n/a | partially starts | partially passed | partially verified | `docker compose --env-file .env.example -f docker-compose.yml config` renders. TMP-030 fixed acquisition-api image build; TMP-031 fixed notification-worker DB env/startup. Remaining runtime blockers include acquisition-api schema bootstrap, notification `message_outbox` schema, and postback-dispatcher DB host. |
...
| compose runtime stack | compose render plus bounded smoke | `docker compose --env-file .env.example -f docker-compose.yml config`; bounded `docker compose ... up -d --build ...` smoke | config renders; smoke reaches app startup and health probes where possible | config pass; acquisition-api image build fixed; notification-worker startup fixed; subscription, notification API, cadence, and landing-web health passed; remaining runtime blockers are schema provisioning and postback DB host | partially verified | live flows still require runtime blocker fixes plus real env/provider values. |
...
| compose runtime start | Config render passes with `.env.example`; temporary isolated auth allows builder-image pulls; bounded smoke reaches app startup; notification-worker startup is fixed, but acquisition-api exits on admin schema bootstrap, notification-worker dispatcher lacks `message_outbox`, and postback-dispatcher uses an invalid localhost DB target. Placeholder values are still not live-flow proof. | Fix service-specific schema/runtime blockers, then rerun bounded compose smoke with real local `.env`/provider values and service health checks. |
...
- Compose config renders with `.env.example`, isolated temporary auth makes builder-image pulls work, and notification-worker startup is fixed, but do not treat compose runtime as verified until acquisition-api schema bootstrap, notification `message_outbox` provisioning, postback-dispatcher DB host, real env/provider values, and all health checks pass.
