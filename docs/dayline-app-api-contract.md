...
- `GET /v1/app/catalog?tenant=careerify&country=GH` (no auth) -> `{"products": [{"slug", "name", "tagline", "description", "category", "artwork_url", "sample_content", "price", "currency", "billing_cycle", "flow_type", "subscriber_count"}]}`.
- Backed by `campaigns` plus new nullable columns `app_name`, `app_tagline`, `app_description`, `app_category`, `app_artwork_url`, `app_sample_content` (single migration, additive only). Fall back to lp_copy fields where app_* is null. Only campaigns with `enabled=true` (existing semantics) are listed.
...
- `POST /v1/app/devices` auth; body `{"fcm_token": "...", "platform": "android"|"ios"}` -> 204. Upsert into new `app_devices` (msisdn, tenant_key, fcm_token unique, platform, updated_at).
